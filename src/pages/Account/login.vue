...
async function login() {
  formRef.value?.validate(async (errors: any) => {
    if (!errors) {
      const response = await fetch('https://bricktopia.jaimymathon.eu/api/authentication/login.php', {
        method: 'POST',
        headers: {
      ...
      const loginResult = data.success

      if (loginResult) {
        // Sla de gebruikersnaam op in de localStorage
        localStorage.setItem('username', loginInfo.value.username)

        useNotifyStore().success(t('login.successMessage'))
        if (loginInfo.value.username.toLowerCase() === 'admin')
          router.push('/dashboard')
        else
          router.push('/shop/products')
        // Redirect naar de startpagina na succesvolle login
      }
      else {
