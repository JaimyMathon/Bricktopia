...
async function register() {
  formRef.value?.validate(async (errors: any) => {
    if (!errors) {
      const response = await fetch('https://bricktopia.jaimymathon.eu/api/authentication/register.php', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify(registerInfo.value),
      })

      const data = await response.json()
      const registerResult = data.success
